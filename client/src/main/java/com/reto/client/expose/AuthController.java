    ...
    public Mono<ResponseEntity<ResponseGeneralDto>> createAuth(@RequestBody AuthDto authDto) {
        ...
        if (!AuthValidator.isValidClientId.test(authDto)) {
            return Mono.just(ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ResponseGeneralDto("ERROR", HttpStatus.BAD_REQUEST.value(),
                            "Error: clientId is invalid", null)));
        }
        ...
        return authService.crearAuth(authDto)
                .map(createdAuth -> {
                    ...
                    ResponseGeneralDto response = new ResponseGeneralDto("SUCCESS", HttpStatus.CREATED.value(),
                            "Authentication created successfully", createdAuth);
                    return ResponseEntity.status(HttpStatus.CREATED).body(response);
                })
                .onErrorResume(ex -> {

                    ResponseGeneralDto response = new ResponseGeneralDto("ERROR", HttpStatus.INTERNAL_SERVER_ERROR.value(),
